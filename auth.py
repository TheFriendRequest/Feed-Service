...
try:
    ...
    if len(firebase_admin._apps) > 0:
        ...
        print("Firebase Admin already initialized in Feed Service")
    else:
        ...
        if os.path.exists(default_path):
            try:
                ...
                _firebase_initialized = True
                print("✅ Firebase Admin initialized with serviceAccountKey.json in Feed Service")
        ...
        if not _firebase_initialized:
            ...
            if project_id:
                try:
                    ...
                    _firebase_initialized = True
                    print("✅ Firebase Admin initialized with Application Default Credentials in Feed Service")
                except Exception as e:
                    print(f"❌ Error initializing Firebase with ADC: {e}")
        
        if not _firebase_initialized:
            print("❌ Firebase initialization FAILED in Feed Service")
            print("   Please ensure serviceAccountKey.json exists in Feed-Service/ directory")
            print("   Or set FIREBASE_SERVICE_ACCOUNT_PATH environment variable")
            
except Exception as e:
    print(f"❌ Firebase initialization error in Feed Service: {e}")
    import traceback
    traceback.print_exc()
    _firebase_initialized = False
